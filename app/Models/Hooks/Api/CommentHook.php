...
class CommentHook
{
   ...
   */
    public function hook_query_index(&$query,$request, $id=0) {
        ...
        \Config::set('constants.PAGINATION_LIMIT', 10);
        $query->join("users", "users.id", "=", "comments.user_id");
        $query->leftJoin("films", "films.id", "=", "comments.film_id");
        $query->select("comments.*","films.slug","users.name");
        if(!empty($request['slug'])) {
            $query->where("films.slug", "=", $request['slug']);
        }
